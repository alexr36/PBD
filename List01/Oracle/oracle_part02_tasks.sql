...
SELECT 
    ...
    TO_CHAR(CASE
        WHEN EXTRACT(DAY FROM w_stadku_od) <= 15
            THEN CASE
                    WHEN '2024-10-29' <= NEXT_DAY(LAST_DAY('2024-10-29') - 7, 'WEDNESDAY')
                        THEN NEXT_DAY(LAST_DAY('2024-10-29') - 7, 'WEDNESDAY')
                    ELSE NEXT_DAY(LAST_DAY(ADD_MONTHS('2024-10-29', 1)) - 7, 'WEDNESDAY')
                 END
            ELSE NEXT_DAY(LAST_DAY(ADD_MONTHS('2024-10-29', 1)) - 7, 'WEDNESDAY')
...
SELECT 
    ...
    TO_CHAR(CASE
        WHEN EXTRACT(DAY FROM w_stadku_od) <= 15
            THEN CASE
                    WHEN '2024-10-31' <= NEXT_DAY(LAST_DAY('2024-10-31') - 7, 'WEDNESDAY')
                        THEN NEXT_DAY(LAST_DAY('2024-10-31') - 7, 'WEDNESDAY')
                    ELSE NEXT_DAY(LAST_DAY(ADD_MONTHS('2024-10-31', 1)) - 7, 'WEDNESDAY')
                 END
        ELSE NEXT_DAY(LAST_DAY(ADD_MONTHS('2024-10-31', 1)) - 7, 'WEDNESDAY')
        END, 'YYYY-MM-DD')             "WYPLATA"
FROM Kocury
