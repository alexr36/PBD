...
CREATE TABLE Bandy (
    ...
    teren      VARCHAR2(15) CONSTRAINT uk_bandy_teren UNIQUE,
    szef_bandy VARCHAR2(15) CONSTRAINT uk_bandy_szef_bandy UNIQUE
);
...
CREATE TABLE Wrogowie (
    ...
    stopien_wrogosci NUMBER(2)    CONSTRAINT ck_wrogowie_stopien_range CHECK (stopien_wrogosci BETWEEN 1 AND 10),
    gatunek          VARCHAR2(15),
    lapowka          VARCHAR2(20)
);
...
CREATE TABLE Kocury (
    ...
    plec            VARCHAR2(1)  CONSTRAINT ck_kocury_plec_md CHECK (plec IN ('M', 'D')),
    pseudo          VARCHAR2(15) CONSTRAINT pk_kocury PRIMARY KEY,
    funkcja         VARCHAR2(10) CONSTRAINT fk_funkcje REFERENCES Funkcje(funkcja),
    szef            VARCHAR2(15) CONSTRAINT fk_kocury_szef REFERENCES Kocury(pseudo),
    w_stadku_od     DATE DEFAULT SYSDATE,
    przydzial_myszy NUMBER(3),
    myszy_extra     NUMBER(3),
    nr_bandy        NUMBER(2)    CONSTRAINT fk_bandy REFERENCES Bandy(nr_bandy)
);

CREATE TABLE Wrogowie_kocurow (
    pseudo         VARCHAR2(15) CONSTRAINT fk_kocury_pseudo REFERENCES Kocury(pseudo),
    imie_wroga     VARCHAR2(15) CONSTRAINT fk_wrogowie REFERENCES Wrogowie(imie_wroga),
